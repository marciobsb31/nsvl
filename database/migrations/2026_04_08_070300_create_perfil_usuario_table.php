...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('perfil_usuario', function (Blueprint $table) {
            ...
            $table->foreignId('usuario_abrangencia_id')->nullable()->constrained('usuario_abrangencia');
            $table->date('data_inicio_vigencia')->nullable();
            $table->date('data_fim_vigencia')->nullable();
            $table->string('origem_tipo', 30);
            $table->foreignId('solicitacao_cadastro_origem_id')
                ->nullable()
                ->index()
                ->constrained('solicitacoes_cadastro');
            ...
            $table->unique(
                ['usuario_id', 'perfil_id', 'usuario_abrangencia_id'],
                'perfil_usuario_full_unique'
            );
        });
    }
