<template>
  <DefaultLayout>
    <HeaderPage title="Detalhar/Avaliar Solicitação de Cadastros" customClass="mb-3">
      <template v-slot:actions>
        <br-button :color-mode="$appTheme" emphasis="primary" aria-label="Cadastrar usuário" @click="router.push('/gerenciar-cadastros')">
          Voltar
        </br-button>
      </template>
    </HeaderPage>
    <Card>
       ...
       <div v-if="detalhe" class="painel-cabecalho-info mb-4">
    ...
    <div v-if="detalhe?.status === 'em_analise' && detalhe?.pode_avaliar !== false" class="painel-secao">
      ...
      <div class="secao-acoes">
        ...
        <button
          ...
          type="button"
          @click="aprovarSolicitacao({ perfilId: perfilSelecionado, vigenciaInicio, vigenciaFim })"
            ...
            <tr v-for="(p, idx) in perfisVinculadosOrdenados" :key="`perfil-${idx}-${p.id ?? idx}`">
              ...
              <td>
                <button
                  ...
                  :class="p.vigente ? 'secondary' : 'success'"
                  type="button"
                  @click="onTogglePerfilVinculado({ perfilUsuarioId: p.id, acao: p.vigente ? 'desativar' : 'ativar' })"
                >
...
defineOptions({ name: 'DetalharSolicitacaoCadastroPage' })
import DefaultLayout from '@/layouts/DefaultLayout.vue'
import HeaderPage from '@/core/components/HeaderPage/HeaderPage.vue'
import Card from '@/core/components/Card/Card.vue'
import { obterSolicitacaoCadastro, type PerfilVinculado, type SolicitacaoCadastroDetalhe } from '@/services/SolicitacaoCadastroService';
import { ref, computed, watch, onMounted, reactive } from 'vue';
import { useRouter } from 'vue-router';
import { aprovarSolicitacao as apiAprovar,
  ...
  adicionarPerfilVinculado as apiAdicionarPerfilVinculado, } from '@/services/GerenciarSolicitacaoCadastroService';
...
const detalhe = ref<(SolicitacaoCadastroDetalhe & { cpf?: string }) | null>(null)
...
const perfisVinculadosLista = computed<PerfilVinculadoExibicao[]>(() => {
  const raw = detalhe?.value?.perfis_vinculados
  if (!Array.isArray(raw) || raw.length === 0) return []
  return raw.map((p, i) => ({
    id: p.id ?? (detalhe?.value?.id ?? 0) * 1000 + i,
    perfil: p.perfil ?? '—',
...
function onSubmitReprovar() {
  ...
  erroReprovar.value = ''
  reprovarSolicitacao({ justificativa: justificativaReprovacao.value.trim() })
  fecharModalReprovar()
}
...
async function detalhar(id: number) {
  if (!id) {
  ...
  try {
    const response = await obterSolicitacaoCadastro(id)
    console.log('[Detalhar] Resposta da API:', response ? { id: response.id, nome: response.nome, perfis: response.perfis_vinculados?.length } : null)
    if (!response?.id) {
      throw new Error('Resposta da API inválida: dados incompletos.')
    }
    detalhe.value = response
...
async function aprovarSolicitacao(payload?: { perfilId?: string | number | null; vigenciaInicio?: string; vigenciaFim?: string }) {
  ...
  try {
    await apiAprovar(detalhe.value.id, payload)
...
async function reprovarSolicitacao(payload: { justificativa: string }) {
  ...
  avaliando.value = true
  try {
    await apiReprovar(detalhe.value.id, payload.justificativa)
...
async function onTogglePerfilVinculado(payload: { perfilUsuarioId: number; acao: 'ativar' | 'desativar' }) {
  ...
  try {
    if (payload.acao === 'ativar') {
      await apiAtivarPerfilVinculado(detalhe.value.id, payload.perfilUsuarioId)
      success('Cadastro ativado com sucesso.')
    } else {
      await apiDesativarPerfilVinculado(detalhe.value.id, payload.perfilUsuarioId)
      success('Cadastro desativado com sucesso.')
    }

    detalhe.value = await obterSolicitacaoCadastro(detalhe.value.id)
  } catch (e: unknown) {
    const msg =
...
async function onAdicionarPerfilVinculado(payload: {
  ...
  avaliando.value = true
  try {
    await apiAdicionarPerfilVinculado(detalhe.value.id, {
